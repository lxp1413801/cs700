...
LRESULT CmodbusTestDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COM_SENDED:
		...
		break;
	case WM_COM_WRITE_CLIB_OVER:
		GetDlgItem(IDC_BUTTON_SET_POINT)->EnableWindow(true);
		if(hThreadModbusRead){
			ResumeThread(hThreadModbusRead);
		}
		if(m_get_list_clib_status())
		{
			OnBnClickedButtonlastpoint();
		}
		else{
			CString str;
			str.Format(__T("当前点：第%d点标定不成功"),FlowClibPoint);
			::AfxMessageBox(str);
		}
		break;
	default:
...
int CmodbusTestDlg::m_modebus_received_process(uint8_t* buf, uint16_t len)
{
	...
	case FUNC_WRITE_FLASH_CLIBDATA_COVER:
		...
		if(i<count){
			m_list.SetItemText(i,2,__T("标定成功"));
			//::update
			writeClibIndex=(int)(buf[0]);
		}
...
*/
	...
	for(i=1;i<modbusAddrCount;i++){
		...
		while(to<2000){
			Sleep(100);
			to+=100;
			if(writeClibIndex==(int)(stp->addr)){
				break;
			}
		}
	}
	...
	::SendMessage(__hwndMain, WM_COM_WRITE_CLIB_OVER,ll, (LPARAM)sendBuf );
	return 1;
}
...
void CmodbusTestDlg::m_clear_list_comm_status(void)
{
	int rowCount=m_list.GetItemCount();
	if(rowCount<=1)return;
	int i=0;
	for(i=1;i<rowCount;i++){
		m_list.SetItemText(i,2,__T("未标定"));
	}
}
bool CmodbusTestDlg::m_get_list_clib_status(void)
{
	int i;
	int rowCount=m_list.GetItemCount();
...
void CmodbusTestDlg::OnBnClickedButtonSetPoint()
{
	...
	*/
	...
	//}
	//CloseHandle(hThreadModebusWriteClib);
		GetDlgItem(IDC_BUTTON_SET_POINT)->EnableWindow(false);
		m_clear_list_comm_status();
