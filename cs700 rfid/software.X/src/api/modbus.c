    ...
    switch( startAddr & 0xff){
		...
		case 0x09:
			if(flashOpStr.op==enFLASH_NOTHING){
				flashOpStr.flashAddr= __get_offset(st_SystemData,price);		
				flashOpStr.len=2;	
				flashOpStr.buf[0]=rbuf[5];
				flashOpStr.buf[1]=rbuf[4];
                if(*((uint16_t*)(flashOpStr.buf))>1000){*((uint16_t*)(flashOpStr.buf))=1000;}
                if(*((uint16_t*)(flashOpStr.buf))<10){*((uint16_t*)(flashOpStr.buf))=10;}
				//*((uint16_t*)(flashOpStr.buf))=*((uint16_t*)(rbuf+4));
				flashOpStr.op=emFLASH_WRITE_main_SYSTEM_DATA;	
			}else{
                modbus_response_illgeal_function(rbuf,4);
                return;
            }
			break;	
        default:
