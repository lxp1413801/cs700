...
void ui_disp_menu(void) {
    ...
    if((valveStatus==VALVE_CLOSEING || valveStatus==VALVE_OPENING || valveStatus==VALVE_CLOSED) && (menu<3)){
             lcd_clear_buffer(); 
            if(valveStatus==VALVE_CLOSED || (sec&0x01)){
               
            if(opReason==OFF_REASON_LA){
                lcd_disp_str((uint8_t*)" la 0ff ");
            }
            if(opReason== OFF_REASON_MAN){
                lcd_disp_str((uint8_t*)"    0ff ");
            }
            if(opReason==ON_REASON_HA){
                lcd_disp_str((uint8_t*)" ha 0n  ");
            }
            if(opReason==ON_REASON_MAN){
                lcd_disp_str((uint8_t*)"    0n  ");
            }
        }else{
            lcd_disp_refresh(); 
        }
