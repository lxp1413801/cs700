...
uint8_t rfid_comm_process_recharge(void){
	...
	piccResult = picc_authenticate(PICC_CMD_MF_AUTH_KEY_B, 15, (st_MifareKey*) userKeyB, &PiccUid);
    if(piccResult!=STATUS_OK)return 0;
	if(mainSystemData.payTimes!=stpDec->Times){
		if(mainSystemData.payTimes==stpDec->Times+1){
			...
			if(stpDec->Value != mainSystemData.transferMoney ){
				return 0;
			}
		}
	}
	...
	//mainSystemData.transferMoney=stpDec->Value;
	//m_mem_cpy_len((uint8_t*)(&flashOpStr.buf[0]),(uint8_t*)(&(__q31)),4);
	m_mem_cpy_len((uint8_t*)(&flashOpStr.buf[0]),(uint8_t*)(cardDecBuf),6);
...
uint8_t cardReadingFlag=0;

uint8_t rfid_comm_process(void) {
	uint8_t valveCtrlflag=0;
    uint8_t ret = 0;
    em_PcdStatusCode result;
	if(deviceStatus.bits.acPowerDown)return 0;
	if(valveStatus==VALVE_OPENING || valveStatus==VALVE_CLOSEING)return 0;
	cardReadingFlag=1;
    do {
		CardError=NO_CARD;
        ...
        if(KeyValue==downKEY_DOWN){
            ...
            led5_on();led2_on();led1_off();led3_off();
	...
	if(valveCtrlflag){
		if(valveStatus==VALVE_OPENED){
			in_app_vavle_off(OFF_REASON_MAN);
			//break;
		}
		if(valveStatus==VALVE_CLOSED){
			if(deviceStatus.bits.noEnough==0){
				in_app_vavle_on(ON_REASON_MAN);
			}
			//break;
		}
	}
