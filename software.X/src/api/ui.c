...
void ui_uint32_2_str(uint8_t* buf, int32_t x, uint8_t len) {
    ...
    while (len--) {
        __t32 = x % 10;
        *buf-- = (uint8_t)hexTable[__t32];
        x /= 10;
    }
}
void ui_head_hide(uint8_t* str, uint8_t loc) {
    while (loc--) {
        if (*str == '0') {
            *str++ = ' ';
        } else {
...
void ui_disp_str_num_s(uint8_t* str, uint16_t num) {
    //uint16_t t;
    lcd_clear_buffer();
    m_mem_set(lcdBuf, ' ', 8);
    lcdBuf[8] = '\0';
    ...
    __t16 = num;
    lcdBuf[7] = '0' + (uint8_t)(__t16 % 10);
    __t16 /= 10;
    if (__t16 > 0) {
        lcdBuf[6] = '0' + (uint8_t)(__t16 % 10);
        __t16 /= 10;
        if (__t16 > 0) {
            lcdBuf[5] = '0' + (uint8_t)(__t16 % 10);
            __t16 /= 10;
            if (__t16 > 0) {
                lcdBuf[4] = '0' + (uint8_t)(__t16 % 10);
            }
        }
    }
    lcd_disp_str(lcdBuf);
...
void ui_start_display(void) {
    ui_disp_hello();
    //ui_disp_logo(9);
    ui_disp_str_dly((uint8_t*) " cs700", 9);
}
//display floq
...
void ui_disp_balance_and_volume(void){
    ...
    if(__q31>=0){
        ...
        ui_uint32_2_str((uint8_t*) (lcdBuf + sizeof (lcdBuf) - 2), __q31, 3);
        ui_head_hide(lcdBuf+4, 2);
        lcdBuf[4]='-';
...
void ui_disp_set_line_reset(void)
{
    ui_disp_str_num((uint8_t*)"l-",mainSystemData.flowRatio);
}
...
void ui_disp_recharge(void){
	//m_mem_set(lcdBuf, '0', sizeof (lcdBuf));
	//__q15=*((int16_t*)(uiAdjbuf));
	//ui_disp_str_num((uint8_t*)"rcg-", __t16);
    //m_mem_cpy_len(lcdBuf,(uint8_t*)"recharg-")
    lcd_disp_str((uint8_t*) "recharge");
}
void ui_disp_modbus_id(void){
	//__t8=*((uint8_t*)(uiAdjbuf));
	ui_disp_str_num((uint8_t*)"addr-", uiAdjbuf[0]);
}
/*
...
void ui_disp_set_price(void){
	__t16=*((uint16_t*)(uiAdjbuf));
	//ui_disp_str_num((uint8_t*)"dj-",__t16);
	ui_disp_str_num_s((uint8_t*)"dj-",__t16);
	lcd_disp_dp(2, true);
	lcd_disp_refresh();
}
...
void ui_disp_clear_card_id(void){
	if(mainSystemData.cardID==0)
		lcd_disp_str((uint8_t*)"card-id0");
	else
		lcd_disp_str((uint8_t*)"card-id9");
}
